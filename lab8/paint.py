...
while running:
    ...
    for event in pg.event.get():
        ...
        if di['right_triangle'] == 1:
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                (x, y) = pos
                pg.draw.rect(screen, 'white', (x, y, ed, ed))
                erase = True
            if event.type == pg.MOUSEMOTION:
                if erase:
                    pg.draw.rect(screen, 'white', (pos[0], pos[1], ed, ed))
            if event.type == pg.MOUSEBUTTONUP:
                erase = False
      
        txt = font.render("square:1 right triangle:2 rhombus:3 equivalent triangle:4 rectangle:5 eraser:6 ", True, (255,0,0))
        screen.blit(txt, (0,0))
    pg.display.update()
pg.quit()
